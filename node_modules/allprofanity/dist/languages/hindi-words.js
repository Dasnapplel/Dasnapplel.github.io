// src/hindi-words.ts
/**
 * Hindi/Hinglish profanity words list
 * Contains common profanities in both Devanagari and Roman scripts
 * ⚠️ Warning: This file contains explicit language in Hindi
 */
const hindiBadWords = [
    // ========================================
    // === Original Seed Words ===
    // ========================================
    // Roman script Hindi/Hinglish profanities
    "behen chod",
    "behenchod",
    "bc",
    "bhenchod",
    "bakchod",
    "chutiya",
    "chutiyapa",
    "gandu",
    "gaandu",
    "harami",
    "haramzada",
    "kutta",
    "kutte",
    "kamina",
    "lund",
    "lauda",
    "loda",
    "randi",
    "saala",
    "madarchod",
    "mc",
    "chod",
    "chodu",
    // Devanagari script profanities
    "भड़वा",
    "भोसड़ी",
    "बहन चोद",
    "भेन चोद",
    "चूतिया",
    "हरामी",
    "हरामज़ादा",
    "कमीना",
    "लंड",
    "लौड़ा",
    "रंडी",
    "साला",
    "मादरचोद",
    // Common misspellings and variations (Original)
    "bhnchd",
    "behanchod",
    "bhanchod",
    "bhen chod",
    "chutia",
    "rendi",
    "maderchod",
    "madarchood",
    "madarjaat",
    "gandoo",
    // ========================================
    // === Expanded List (Roman Script) ===
    // ========================================
    // --- Variations of 'Behenchod' ---
    "behanchood", "behenchood", "bhenchood", "bhainchod", "bhain chood", "bahenchod",
    "behnchod", "behncod", "behn chod", "bhen cod", "bhen chood", "bahanchod", "bhenco",
    "bhencho", "b c", "be si", "bee see", "behenkechodo", "bhenkelode",
    "bkl",
    "bahan chod", "bahan ke",
    // --- Variations of 'Madarchod' ---
    "madar chod", "madarchodd", "madarchoddd", "madar chood", "madrchod", "madarchud",
    "mader chod", "maderchood", "maderchod", "modarchod", "mother chod",
    "motherchodd", "mother fucker",
    "maachod", "maa chod", "ma chod", "maa chood", "m c", "em cee", "em see",
    "madarjaad", "madarzaat", "madarjatt", "madarchom",
    "teri maa chod", "teri ma ki chut",
    // --- Variations of 'Chutiya' ---
    "chutiyaa", "chutiye", "chuthiya", "chutya", "chutiyaap", "chutiyap", "chutyapa",
    "chutiyapaa", "chutiyapanti", "chutiyapnatee", "chutiyapanti", "chootiya", "chootya",
    "chootiye", "chootiyapa", "chut", "choot", "chuchiya",
    "chodu ram",
    "chutmarike",
    "chut ke",
    "maha chutiya",
    "ek number ka chutiya",
    "asli chutiya",
    // --- Variations of 'Gandu' / 'Gaand' ---
    "gandu lal",
    "gandul", "ganduu", "gand", "gaand", "g@ndu", "g@nd", "gaandfat",
    "gandfattu", "gaand mara",
    "gaand marwa",
    "gaandmasti",
    "gandu giri", "gaandu giri", "gandu augmentation",
    "gandfaad",
    "gandu bachcha",
    "teri gaand mein",
    // --- Variations of 'Lund' / 'Lauda' ---
    "laund", "lawda", "lora", "laura", "lulla",
    "lund choos",
    "lund fakir",
    "lund hilana",
    "lund ka baal",
    "lundtopi",
    "laude",
    "laude lag gaye",
    "lauda lasan",
    "mere lund se",
    // --- Variations of 'Bhosda' / 'Bhosdike' ---
    "bhosda",
    "bhosada", "bhosra", "bhosrika", "bhosarike", "bhosdike",
    "bhoshdike", "bhosdi ke",
    "bhosdiwala", "bhosdiwale", "bhosdiwaale", "bhosadchod",
    "bhosad",
    "teri maa ka bhosda",
    // --- Variations of 'Randi' ---
    "randvi", "randwe",
    "randibaaz", "randibaazi", "randibaj", "randi ka",
    "randi ki aulad",
    "randi khana",
    "chakke",
    "gashti",
    "patur",
    "beswa",
    // --- Variations of 'Harami' / 'Haramzada' ---
    "haraami", "harami pilla",
    "haramipana",
    "haramzade", "haramzadeh", "haram zaada", "haramjada", "haramjaade",
    "haramkhor", "haraamkhor", "haram ka",
    "haram ka pilla",
    // --- Variations of 'Kamina' ---
    "kameena", "kaminay", "kaminey", "kaminapan", "kaminpana", "kamini",
    // --- Variations of 'Kutta' ---
    "kutte kamine",
    "kutti", "kutiya", "kutya", "kuttiya", "kutri",
    "kutte ki maut",
    "kutte ka pilla",
    "gali ka kutta",
    "kutte ki aulad",
    // --- Variations of 'Saala' ---
    "sale", "saale", "saaley", "sala kutta", "sala harami",
    "saale chutiye",
    // --- Variations of 'Chod' / 'Chodu' ---
    "chodd", "chhod",
    "chodumal",
    "choduram", "chhodu", "chud",
    "chudai",
    "chudwa",
    "chudakkad",
    "chudasi",
    "chudasa",
    "chudwana", "chudwaana", "chudwaya",
    "chodne",
    // --- Variations of 'Bakchod' / 'Bakchodi' ---
    "bakchodd", "bakchodi", "bakchodiya", "bakchodhi", "bakchodi pelna",
    "bakait",
    "bakwaas",
    // --- Bodily Functions / Parts (Vulgar) ---
    "jhaant", "jhaat", "jhant", "jhantu", "jhatu", "jantoo", "jhaat barabar",
    "jhaat ka baal",
    "tatte", "tatta", "tatti", "tati", "tat",
    "tatti khana",
    "tatti surat",
    "goo", "gu", "goobar",
    "mut", "moot", "mutra",
    "mutna",
    "peshab",
    "muth", "mutthi",
    "muthal",
    "hastmaithun",
    // --- Slurs & Related Insults ---
    "hijra", "hijda", "hijraa", "hizra", "hijjra",
    "chakka", "chhakka", "chaka",
    "namard", "naamard", "namardangi",
    "napunsak", "napunsakta",
    "khusra",
    "khassi",
    "launda",
    "laundiya",
    "laundebaaz",
    // --- General Insults & Offensive Terms ---
    "bhadwa",
    "bhadwe",
    "bharwa", "bhadua", "bhadva",
    "dalal",
    "dalla", "dalaal",
    "besharam",
    "besharm", "besaram", "besharmi",
    "behaya",
    "behayaa", "behayaai",
    "neech",
    "nich", "neech admi",
    "paaji",
    "paji", "pajji",
    "badmaash",
    "badmash", "badmashi",
    "luchha",
    "lucha", "luchchai",
    "lafanga",
    "lafange", "lafangey", "lafantar",
    "awaara",
    "gadha",
    "gadhe", "gadhi",
    "ullu",
    "ullu ka pattha",
    "ullu ki patthi",
    "bandar",
    "suar",
    "suwar", "soar", "pig",
    "suar ki aulad",
    "janwar",
    "shaitan",
    "rakshas",
    "chichora",
    "charsi",
    "ganjedi",
    "bewakoof", "bevakoof", "bewaqoof",
    "murkh",
    "paagal", "pagal", "pagla", "pagli",
    "dekh lunga",
    "aukat",
    "aukaat",
    "chapri",
    "nibba", "nibbi",
    "dhakkan",
    "paltu",
    "chamcha",
    "bhikari",
    "kanjoos",
    "makhi choos",
    "phattu",
    "darpok",
    "kayar",
    "ghatiya",
    "khota",
    "haram ka khana",
    "muh kala karna",
    "izzat lutna",
    "phitte muh",
    "dur fitte muh",
    "mar ja",
    "khudkushi kar le",
    "joota maru",
    "joota",
    "nalayak",
    "nikamma",
    "kamchor",
    "gawar",
    "jungli",
    // --- Compound Insults & Phrases ---
    "madarchod gandu", "gandu madarchod", "behenchod chutiya", "chutiya behenchod",
    "saala kutta kamina", "harami kutta", "kaminey suar", "randi ki beti",
    "bhadwe ki aulad", "bhosdi ke kutte", "lund fakir chutiya", "chutiya sala", "gandu sala",
    "abe saale", "oye harami", "chal be chutiye", "nikal laude",
    "teri maa ka yaar",
    "teri behen ka yaar",
    "baap ka maal",
    "gandi naali ke keede",
    "tere baap ka naukar",
    "khandan pe mat ja",
    "behen ke lode",
    "teri maa ki",
    // --- More Obscure/Regional/Figurative ---
    "chutiya sala gandu",
    "bhen ke takke",
    "khatey",
    "pataka",
    "maal",
    "item",
    "topibaaz",
    "dhongi",
    "pakhandi",
    "ghoos khor",
    "char sau bees",
    "dedh shana",
    "bitodi",
    "chidimar",
    "natak karna",
    "raand",
    // ========================================
    // === Expanded List (Devanagari Script) ===
    // ========================================
    "बहनचोद", "भैनचोद", "भेनचोड", "बहेनचोद", "बी सी", "बकलोल",
    "मादर चोद", "मादरचूड", "मदरचोद", "माचोद", "मादरजात", "तेरी माँ चोद", "तेरी माँ की चूत", "एम् सी",
    "चूतिये", "चूतियापा", "चूतियापंती", "चूत", "चूचीया", "चूतड़",
    "चूतर",
    "चूत मारीके", "महा चूतिया", "असली चूतिया",
    "गांड", "गांद", "गंड", "गांडफत", "गांडू गिरी", "गांड मरा", "गांड मरवा", "तेरी गांड में",
    "लौड़ा", "लोड़ा", "लवड़ा", "लंड चूस", "लंड का बाल", "लौड़े", "मेरे लंड से",
    "भोसड़ा", "भोसड़ीके", "भोसडीके", "भोस्डिके", "भोसड़ी वाला", "भोसड़ी वाले", "तेरी माँ का भोसड़ा",
    "रंडीबाज़", "रंडी की औलाद", "रंडीखाना", "गश्ती", "बेसवा",
    "हरामीपना", "हरामज़ादे", "हरामखोर", "हराम का",
    "कमीने", "कमीनी", "कमीनापन",
    "कुत्ती", "कुतिया", "कुत्ते की मौत", "कुत्ते का पिल्ला", "गली का कुत्ता", "कुत्ते की औलाद",
    "साले", "साले कुत्ते",
    "छोड़", "चोदू", "चुदक्कड़", "चुदाई", "चुदवाना", "चोदने",
    "बकचोदी", "बकैत", "बकवास",
    "झांट", "झाँट", "झाटू", "झाँटू", "टट्टे", "टट्टी", "गू", "गोबर", "मूत", "मूत्र", "पेशाब", "मूठ", "मुठ्ठी", "मुठल", "हस्तमैथुन",
    "हिजड़ा", "छक्का", "नामर्द", "नपुंसक", "खुसरा", "खस्सी", "लौंडा", "लौंडिया", "लौंडेबाज़",
    "भरवा", "दलाल", "दल्ला", "बेशर्म", "बेशर्मी", "बेहया", "नीच", "पाजी", "बदमाश",
    "बदमाशी", "लुच्चा", "लफंगा", "आवारा", "गधा", "गधी", "उल्लू", "उल्लू का पट्ठा", "बंदर",
    "सूअर", "सुअर", "सूअर की औलाद", "जानवर", "शैतान", "राक्षस", "चिचोरा", "चर्सी", "गंजेड़ी",
    "बेवकूफ", "मूर्ख", "पागल", "पगला", "पगली", "देख लूँगा", "औकात", "ढक्कन", "पलटू", "चमचा",
    "भिखारी", "कंजूस", "मक्खीचूस", "फट्टू", "डरपोक", "कायर", "घटिया", "खोटा", "नालायक",
    "निकम्मा", "कामचोर", "गंवार", "जंगली", "फिट्टे मुह", "मर जा", "जूता मारूं",
    "मादरचोद गांडू", "बहनचोद चूतिया", "साला कुत्ता कमीना", "हरामी कुत्ता", "कमीने सूअर",
    "रंडी की बेटी", "भड़वे की औलाद", "भोसड़ी के कुत्ते", "चूतिया साला", "अबे साले", "निकल लौड़े",
    "तेरी माँ का यार", "तेरी बहन का यार", "गंदी नाली के कीड़े", "तेरे बाप का नौकर",
    "बहन के टके",
    "खाते",
    "पटाका",
    "माल",
    "आइटम",
    "टोपीबाज़",
    "ढोंगी",
    "पाखंडी",
    "घूसखोर",
    "चार सौ बीस",
    "डेढ़ शाणा",
    "रांड",
    "तेरी माँ की",
    "बहन के लोड़े", // Behen ke lode (Sister's dick - offensive)
    // Add more words
];
export default hindiBadWords;
//# sourceMappingURL=hindi-words.js.map